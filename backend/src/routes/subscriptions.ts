...
const subscriptionRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    ...
    const { userAddress } = request.query as any;
    const { page, limit, status, sortBy, sortOrder } = request.query as any;
    ...
    try {
      const factory = await blockchainService.getSubscriptionFactory();
      ...
      for (const address of subscriptionAddresses) {
        try {
          const contract = await blockchainService.getSubscriptionContract(address);
          const [terms, status, nextPaymentDue, currentPeriod, totalPaid, totalFromPool] = 
            await contract.getSubscriptionInfo();
  ...
  fastify.get('/:subscriptionId', {
    ...
    try {
      const contract = await blockchainService.getSubscriptionContract(subscriptionId);
      const [terms, status, nextPaymentDue, currentPeriod, totalPaid, totalFromPool] = 
        await contract.getSubscriptionInfo();
  ...
  fastify.post('/', {
    ...
    const { agreement, providerSignature, userSignature } = request.body as any;

    try {
      // Validate signatures
      const factory = await blockchainService.getSubscriptionFactory();
      const domain = blockchainService.getDomain(process.env.SUBSCRIPTION_FACTORY_ADDRESS!);
      const types = blockchainService.getPaymentAgreementTypes();

      const isProviderSigValid = blockchainService.verifySignature(
        domain,
        types,
        agreement,
        providerSignature,
      ...
      if (userAgentAddress !== '0x0000000000000000000000000000000000000000') {
        ...
        const userAgent = await blockchainService.getContract(userAgentAddress, [
          'function isValidSignature(bytes32 hash, bytes signature) external view returns (bytes4)'
        ]);
        
        const hash = blockchainService.getDomain(process.env.SUBSCRIPTION_FACTORY_ADDRESS!);
        // This would need proper EIP-712 hash calculation
        // Simplified for this example
        isUserSigValid = true;
      } else {
        // Verify direct user signature
        isUserSigValid = blockchainService.verifySignature(
          domain,
          types,
          agreement,
          userSignature,
      ...
      const tx = await factory.createSubscription(
        agreement,
        providerSignature,
        userSignature
      );
      ...
      const subscriptionCreatedEvent = receipt.logs.find((log: any) => {
        try {
  ...
  fastify.post('/:subscriptionId/cancel', {
    ...
    const { subscriptionId } = request.params as { subscriptionId: string };
    const { signature, nonce } = request.body as any;
  ...
  fastify.get('/:subscriptionId/payments', {
    ...
    try {
      ...
      const payments = paymentHistory.map((payment: any, index: number) => ({
        period: index + 1,
