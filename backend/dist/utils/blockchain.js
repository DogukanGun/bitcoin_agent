...
class BlockchainService {
    ...
    initialize() {
        ...
        try {
            ...
            this.platformSigner = new ethers_1.ethers.Wallet(signerKey, this.provider);
            this.initialized = true;
    ...
    async getSubscriptionContract(address) {
        const abi = [
    ...
    async signPaymentAgreement(agreement) {
        const signer = await this.getPlatformSigner();
    ...
    verifySignature(domain, types, data, signature, expectedSigner) {
        try {
            const recoveredAddress = ethers_1.ethers.verifyTypedData(domain, types, data, signature);
    ...
    generateAgreementId(user, provider, token, amount, startDate) {
        return ethers_1.ethers.keccak256(ethers_1.ethers.AbiCoder.defaultAbiCoder().encode(['address', 'address', 'address', 'uint256', 'uint256'], [user, provider, token, amount, startDate]));
    }
    ...
    async getSubscriptionEvents(fromBlock = 0, toBlock = 'latest') {
    ...
    async getPaymentEvents(contractAddress, fromBlock = 0) {
