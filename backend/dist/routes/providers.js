...
const providerRoutes = async (fastify) => {
    // Register new provider
    fastify.post('/register', {
        preHandler: [auth_1.authenticateJWT],
        schema: {
            tags: ['Providers'],
        ...
        const { address, name, description, website, email } = request.body;
        try {
            const db = mongodb_2.mongoService.getDb();
    ...
    fastify.get('/', {
        ...
        const { page = 1, limit = 20, verified } = request.query;
        try {
            const db = mongodb_2.mongoService.getDb();
            const providersCollection = db.collection('providers');
            const filter = {};
    ...
    fastify.put('/:providerId', {
        ...
        try {
            ...
            const result = await providersCollection.findOneAndUpdate({ _id: new mongodb_1.ObjectId(providerId) }, { $set: updateData }, { returnDocument: 'after' });
            if (!result) {
                return reply.status(404).send({
                    error: {
                        code: 'PROVIDER_NOT_FOUND',
