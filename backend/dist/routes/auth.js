...
const authRoutes = async (fastify) => {
    ...
    fastify.post('/login', {
        ...
        try {
            ...
            const db = mongodb_1.mongoService.getDb();
            ...
            if (!user) {
                ...
                // Update last login
                await usersCollection.updateOne({ address: address.toLowerCase() }, { $set: { updatedAt: new Date() } });
            }
            // Generate JWT token
            const token = (0, jwt_1.generateToken)(user._id.toString(), user.address);
            return {
                token,
                user: {
